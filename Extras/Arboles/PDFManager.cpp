...
static void demoThree(PDF &p, Tree tree_)
{
   ...
   {
      cout << "Su arbol esta vacio, los datos estaran vacios " << endl;
   }
   else
   {
      root = std::to_string(tree_.getRoot()->getValue());
      weight = std::to_string(pd.weigthTree(tree_.getRoot()));
      height = std::to_string(pd.HeigthTree(tree_.getRoot()));
      inOrder = "";
      preOrder = "";
      postOrder = "";
   }
      ...
      p.drawCircle(136, 600, 30);
      p.setFont(PDF::COURIER_OBLIQUE, 16);
      p.showTextXY(root, 130, 600);
      ...
      p.showTextXY("Datos del arbol", 306, 615);
      p.showTextXY("Peso:", 306, 600);
      p.showTextXY("Profundidad : ", 306, 585);

      p.showTextXY("Arbol en InOrden: ", 90, 525);
      p.showTextXY("Arbol en PreOrden: ", 90, 495);
      p.showTextXY("Arbol en PostOrden: ", 90, 465);
      p.showTextXY("Raiz principal(Padre)", 90, 635);
      ...
      p.showTextXY(weight, 406, 600);
      p.showTextXY(height, 406, 585);
      //****************************************
      // Pie de página
...
int PDFManager::HeigthTree(Node *aux)
{
   if (aux == NULL)
      return 0;
   if (aux->getLeftNode() == NULL && aux->getRightNode() == NULL)
      return 1;
   return __max(HeigthTree(aux->getLeftNode()), HeigthTree(aux->getRightNode())) + 1;
}

int PDFManager::weigthTree(Node *tree)
{
   if (tree == nullptr)
      return 0;
   else
      return 1 + weigthTree(tree->getLeftNode()) + weigthTree(tree->getRightNode());
}
