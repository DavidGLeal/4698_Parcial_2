...
void ArbolB::agregar(int a){
	...
	if( lugarEncontrado ){ // si el lugar encontrado existe
		//se agrega en la pagina encontrada
...
void ArbolB::sumarPrimos(){
	...
	p=principio;
	if(!p){
		cout<<"No existen numeros primos en el arbol"<<endl;
	}	
	while(p){		
		if(p->siguiente){		
			if (comprobarPrimo(p->direccionArbol->valor)){
				sum += p->direccionArbol->valor;	
			}		
		}else{
			if (comprobarPrimo(p->direccionArbol->valor)){
				sum += p->direccionArbol->valor;	
	...
	cout<<"Suma de los numeros primos: "<< sum <<endl;
}

bool ArbolB::comprobarPrimo(int numero){
	bool resp = true;
...
int ArbolB::getCuantos(){
	return this->cuantos;
}

int ArbolB::getNivelMax(){
	return this->nivelMax;
}
 

void ArbolB::setNivelMax(int _nivelMax){
	this->nivelMax=_nivelMax;
}  

void ArbolB::recorrerPaginas(Pagina *pag, int cont){
	cout<<"\n";
	if (pag->obtenerCuantos()>0)
	{
		if (pag->obtenerCasillaEnPosicion(pag, 1)->hizq){
			
			recorrerPaginas(pag->obtenerCasillaEnPosicion(pag, 1)->hizq,1);
			pag->obtenerCasillaEnPosicion(pag, 1)->hizq->pintar();
			cout<<"/*/";
		}
			
		for (int i = 1; i <= pag->obtenerCuantos(); i++){
			if (pag->obtenerCasillaEnPosicion(pag, i)->hder){
			
			recorrerPaginas(pag->obtenerCasillaEnPosicion(pag, i)->hder,1);
			pag->obtenerCasillaEnPosicion(pag, i)->hder->pintar();
			cout<<"/**/";
		}
		}	
	}
...
void ArbolB::ponerNivelPaginas(Pagina *pag, int cont){
	setNivelMax(cont); 
	pag->setNivelPag(cont); 
	if (pag->obtenerCuantos()>0)
	{
		if (pag->obtenerCasillaEnPosicion(pag, 1)->hizq){
			ponerNivelPaginas(pag->obtenerCasillaEnPosicion(pag, 1)->hizq,cont+1);
			
		}
		
		for (int i = 1; i <= pag->obtenerCuantos(); i++){
			if (pag->obtenerCasillaEnPosicion(pag, i)->hder){ 
			ponerNivelPaginas(pag->obtenerCasillaEnPosicion(pag, i)->hder,cont+1);
...
void ArbolB::ponerNivelArbol(){
ponerNivelPaginas(this->raiz,1); 
}

void ArbolB::graficar(Pagina *pag,int nivel){
	
	if(pag->getNivelPag()==nivel){
				pag->pintar();
				}
		if (pag->obtenerCuantos()>0)
		{
			...
			if (pag->obtenerCasillaEnPosicion(pag, 1)->hizq){				
				graficar(pag->obtenerCasillaEnPosicion(pag, 1)->hizq,nivel);
			...
			for (int i = 1; i <= pag->obtenerCuantos(); i++){
				...
				graficar(pag->obtenerCasillaEnPosicion(pag, i)->hder,nivel);
				}
			}	
		}
...
void ArbolB::graficarArbol(){
	int esp=0;
	esp=getNivelMax();
	for(int i=1;i<=getNivelMax();i++){
		graficar(this->raiz,i);
		cout<<"\n\n\n";
	}	
} 
//------------------------------------------------------
